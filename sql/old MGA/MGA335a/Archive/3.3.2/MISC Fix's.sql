-- Prevent flight well fomr moving
UPDATE `creature_template` SET `unit_flags`='4' WHERE `name` LIKE '%lightwell%';

-- same update for JC healing pets
UPDATE `creature_template` SET `unit_flags`='4' WHERE `name` LIKE '%stone stat%';
